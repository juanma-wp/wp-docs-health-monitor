...
**Evidence-slot shape for `nonexistent-name` and `broken-example`**: these two drift types have a natural "answer" (the missing identifier, a corrected snippet) that does NOT belong in the evidence slots. The slots are for verifiable doc/code text only. If you put the answer in a slot, the verbatim check rejects it and the entire finding is dropped silently. When you cannot fill a slot with verifiable text, set it to an empty string (`""`) and put the corrective content in `suggestion`. An empty slot is acceptable; a fabricated slot is not.

- **`nonexistent-name`, when a name exists in code but is missing from the doc**: `codeSays` quotes the code site that introduces the name (signature, definition, parameter tag, or schema entry — whichever holds the surface contract for that identifier). `docSays` quotes the surrounding doc passage that frames the absence — the property table header, the API-signature paragraph, the list of documented peers where the missing name would naturally belong. NEVER put the missing identifier itself in `docSays`; by definition, that identifier is not in the doc. If no surrounding passage exists, set `docSays` to `""`.

- **`broken-example`, when a doc snippet would fail at runtime or is syntactically invalid**: `docSays` quotes the broken snippet from the doc verbatim. `codeSays` is one of: (a) a real, working usage of the same API quoted verbatim from the codebase, when the snippet misuses an existing API; or (b) `""`, when the snippet is broken on its own face (syntax error, malformed literal, undefined variable) and no comparison code is needed to demonstrate the breakage. NEVER fabricate "what the code should say". If the corrected version is not literally present in the source files, leave `codeSays` empty and put the fix in `suggestion`.
