...
export function registerBlockType( name, settings ) {
	...
	const preloadedBlock = window.__experimentalBlocks?.[ name ];
	if ( preloadedBlock ) {
		settings = { ...preloadedBlock, ...settings };
	}
	...
	// Canonicalize icon: if not set, use the block name.
	if ( ! settings.icon ) {
	...
	if ( settingsName && settingsName !== name ) {
		console.error(
			'Block names defined in `name` property in block settings (`' +
				settingsName +
				'`) must match the `name` argument passed to `registerBlockType()` (`' +
				name +
				'`).'
		);
		return;
